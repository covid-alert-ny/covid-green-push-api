...
async function notify(server, options, done) {
  ...
  const generateVerificationCode = async (log, onsetDate, testType) => {
    try {
  ...
  const handler = async request => {
    ...
    // THIS IS FOR NYC TESTING ONLY. THIS MUST BE REMOVED AFTER THEY CONFIRM.
    //
    const formattedExpiration = format(codeExpiration, 't')
    console.log(
      `NYC DEBUG Capture. code: ${code}, resolvedOnsetDate: ${resolvedOnsetDate}, codeExpiration: ${codeExpiration}, formattedExpiration: ${formattedExpiration}, smsSent: ${smsSent}, parsedMobile: ${parsedMobile}`
    )
    // REMOVE REMOVE REMOVE REMOVE
    //
    ...
    return {
      ...
      expiresAtTimestamp: formattedExpiration, // format(codeExpiration, 't'),
      smsSent
    }
  }
